...
function createColoredIcon(color: string) {
  return L.divIcon({
    ...
    iconSize: [32, 41],
    iconAnchor: [16, 41], // Point bas du marqueur custom
    popupAnchor: [0, -41],
  });
}
...
export default function PublicMap() {
  ...
  console.log('[PublicMap] premier signalement:', signalements[0]);
  ...
  return (
    <div className="relative h-screen w-full pt-16" style={{ isolation: 'isolate' }}>
      {/* Filtres */}
      <div className="absolute top-20 left-1/2 -translate-x-1/2 z-[1000] flex gap-2 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 px-4 py-3">
        <select
          value={statut}
          ...
          >
            <Popup>
              <div className="min-w-[220px] space-y-2">
                ...
                {s.adresse && (
                  <p className="text-xs text-slate-400 line-clamp-2">{s.adresse}</p>
                )}
